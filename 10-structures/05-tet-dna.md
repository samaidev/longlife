...
### 5.1a "时钟回拨"的分子规模感：TET 如何撬动数百个位点

表观时钟（如 Horvath 时钟）监测 353 个 CpG 位点，而重编程要"回拨"的是这些位点乃至全基因组的甲基化状态。TET 如何完成这一浩大工程？需要建立"分子规模感"：

**工作量估算**。人类基因组约有 2800 万个 CpG 位点（其中约 70% 被甲基化）——TET 介导的重编程去甲基化不需要"全擦"，只需重置"与衰老相关的差异甲基化位点"（数千到数万个）。即便如此，这也是每秒数千次催化反应的规模——**单细胞内的 TET 酶必须高速工作**，这依赖三个条件：①TET 表达上调（OSK 转录激活）；②底物（5mC）丰富（基因组 70% 甲基化）；③2OG/Fe²⁺ 充足（线粒体代谢供给，呼应"线粒体-表观轴"）。

**"位点选择性"的假象与真相**。时钟位点（如 Horvath 353 CpG）只是"探针"——TET 本身没有"只擦时钟位点"的特异性（它氧化所有可及的 5mC）。所谓"时钟回拨"是**全基因组去甲基化浪潮中，时钟位点恰好被同步重置**的结果。**这也解释了重编程的"剂量-安全"问题**：TET 过度激活会擦除"不该擦的"甲基化（如印迹基因、转座子抑制位点）→ 基因组不稳定——"TET 活性必须适度"是表观重编程安全的核心约束（呼应 [01-concept](../08-frontiers/01-reprogramming/01-concept.md) 3.5 节的四层防御）。

**"部分重编程"的结构含义**：短暂/周期性的 OSK 表达 → TET 短暂激活 → 只完成"一轮"去甲基化（擦除衰老相关位点）→ 因子撤去、TET 回落——**"脉冲式 TET 激活"实现"部分擦除"**，既回拨时钟又不致全基因组失控。这是"部分重编程"在分子层面的本质：不是"部分激活 TET"，而是"短暂激活 TET"。
