...
### 3.3a 从"甲基朝向"到"选择性氧化"：一把钥匙开一把锁

TET 的底物选择性看似简单（"只氧化 5mC"），但实现这一选择性的结构机制相当精妙：

**"甲基朝向"的几何逻辑**。晶体结构显示 5mC 的甲基（-CH₃，位于嘧啶环 C5 位）直指 Fe(II) 催化中心。这个朝向意味着：**催化发生时，ferryl 氧攻击的目标（甲基的 C-H 键）必须精确位于"夺氢距离"（约 2.5-3 Å）内**。未修饰的胞嘧啶（C）在 C5 位是氢原子（-H）——氢原子虽小，但缺乏"伸向催化中心的碳原子"，ferryl 无法有效夺氢（C-H 键的键能与空间位置都不匹配）。**"甲基 = 合适的把手，氢 = 不合适的把手"**——TET 通过几何而非化学选择性区分 5mC 与 C。

**"产物不脱落"的再审视**。5mC 氧化为 5hmC 后，C5 位变成羟甲基（-CH₂OH）——这个基团比甲基略大但仍在活性腔的容受范围内（"活性腔容纳 5mC 衍生物"的结构事实）。羟甲基同样朝向 Fe 中心，可被再次氧化（脱氢为醛基）——**"一步一个把手，每步氧化后把手仍在"**是多步催化的几何基础。

**CpG 识别的"上下文锁定"**。TET2 不仅识别 5mC 本身，还识别其所在的 CpG 二核苷酸上下文：5mC 3' 端的鸟嘌呤（G）与 TET2 的残基形成氢键/碱基堆叠接触——"看住 G 才能锁定 C"。**这一上下文偏好的生物学意义**：基因组中 5mC 主要存在于 CpG 位点（基因启动子/基因体），TET2 的 CpG 偏好使其"精准打击"基因组的甲基化热点，而非随机擦除。

**为什么 5hmC 不是"终点"而是"中转站"**：5hmC 既可以被 TET 继续氧化（→5fC→5caC，去甲基化路径），也可以作为独立表观标记被"读取"（5hmC 有自己的结合蛋白）——"5hmC 是去甲基化的中转站，也是独立信号"，这使 TET 的氧化产物具有双重功能（呼应 [01-foundations/02-aging-clocks.md](../01-foundations/02-aging-clocks.md) 的 5hmC 衰老标志物讨论）。
...
**"结构基因组学"的方法论闭环**：TET2 的癌症突变分析展示了结构生物学的"下游应用"——基因组测序发现突变（TCGA/ICGC）→ 结构映射定位突变的功能影响（本文第 4 节）→ 功能实验验证（酶活性/蛋白稳定性）→ 指导预后判断与靶向策略。**"从序列到结构再到功能"的闭环**是现代基因组医学的标准范式，也是 [08-frontiers/04-ai-drugs/01-ai-discovery.md](../08-frontiers/04-ai-drugs/01-ai-discovery.md) 讨论的"AI+结构+组学"融合的基础。对衰老研究而言，同样的方法可用于"衰老相关表观突变的解读"（如克隆性造血 CHIP 的 DNMT3A/TET2 突变，见 [04-immunosenescence](../03-immunity/04-immunosenescence.md) 的 CHIP 讨论）——"结构解释突变、突变指导干预"。
...
TET 介导的去甲基化在抗衰中是"橡皮擦"（擦除衰老甲基化），但在肿瘤中是"双刃剑"——理解其结构基础有助于把握安全边界：

- **去甲基化的"有益面"（抗衰/重编程）**：TET 氧化 5mC → 擦除衰老相关甲基化 → 时钟回拨、基因表达年轻化（Lu 2020 的视神经再生、Macip 2024 的寿命延长均依赖 TET 通路）——"适度去甲基化 = 表观复位"；
- **去甲基化的"有害面"（肿瘤）**：TET 过度激活或异常表达 → 基因组广泛去甲基化 → 转座子激活（LINE-1 去抑制 → 基因组不稳定）、原癌基因去甲基化激活、印迹丢失——"全基因组去甲基化 = 基因组灾难"（这解释了 TET 既是抑癌因子（TET2 失活 → 髓系肿瘤）又可能促癌（过度激活）的矛盾）；
- **"剂量窗口"的结构本质**：TET 的催化活性（Fe(II)/2OG 供给）与表达水平（转录调控）共同决定去甲基化程度——**"适度 TET 活性 = 表观年轻化；过度 TET 活性 = 基因组灾难"**是表观重编程安全的核心约束（呼应 [01-concept](../08-frontiers/01-reprogramming/01-concept.md) 3.5 节的"四层防御"中"监测层"的设计逻辑——实时追踪去甲基化程度防止越界）。

**对"永生药"的启示**：表观重编程要成为安全的"永生药突破口"，必须解决"TET 激活的位点选择性"——理想状态是"只擦衰老相关位点、不碰转座子/印迹位点"，但 TET 本身无位点选择性（氧化所有可及 5mC）。**工程化方向**：①将 TET 靶向特定基因组区域（融合 DNA 结合域）；②脉冲式激活（短暂一轮擦除）；③与转座子抑制机制（H3K9me3/SIRT6）协同——这是 [09-immortality/02-breakthrough-candidates](../09-immortality/02-breakthrough-candidates.md) 中表观重编程 31/40 分背后"工程瓶颈"的结构注脚。
